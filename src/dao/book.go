...
func (b *Book) LoadBooksByUser(userId string)(*[]Book) {
	log.GetLogInstance().Debug("loadBooksByUser", log.Data("userId", userId))
	if len(userId) == 0 {
		log.GetLogInstance().Fatal("user Id is empty")
		return nil
	}
	if db.SqlDb != nil {
	...
	//
	rows, err := db.SqlDb.Query("select id,userId,friendName,friendMobile from tbl_fas_books where userId=? order by friendName", userId)
	if err != nil {
...
func (b *Book) SaveOrUpdate()(bool,error){
	...
	if len(b.Id) == 0 {//新增
		...
		//
		_, err := db.SqlDb.Exec("insert tbl_fas_books(id,userId,friendName,friendMobile) values(?,?,?,?)",
	...
	//更新
	_, err := db.SqlDb.Exec("update tbl_fas_books set userId=?,friendName=?,friendMobile=? where id=?",
...
func (b *Book) RemoveByUser(userId string)(bool, error){
	...
	//删除数据
	_, err := db.SqlDb.Exec("delete from tbl_fas_books where userId=?", userId)
	if err != nil {
		log.GetLogInstance().Fatal(err.Error())
		return false, err
	}
...
func (ub *UserBook) SaveOrUpdate() (bool, error){
	...
	if len(ub.UserId) == 0 || len(ub.BookId) == 0 {
		log.GetLogInstance().Fatal("userId or bookId is empty")
		return false, errors.New("userId or bookId is empty")
	}
	//检查是否存在
	var result bool
	err := db.SqlDb.QueryRow("select count(0) > 0 from tbl_fas_user_books where userId=? and bookId=?", ub.UserId, ub.BookId).Scan(&result)
	if err != nil {
		log.GetLogInstance().Fatal(err.Error())
		return false, err
	}
	...
	//新增
	_, err = db.SqlDb.Exec("insert into tbl_fas_user_books(id,userId,bookId) values(?,?,?)", ub.Id, ub.UserId, ub.BookId)
	if err != nil {
		log.GetLogInstance().Fatal(err.Error())
		return false, err
	}
